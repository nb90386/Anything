/**
 * Portable schema for PolyAlpha Lab.
 *
 * Written to run IDENTICALLY on SQLite (better-sqlite3) and Postgres/Supabase:
 *  - TEXT UUID primary keys (generated in app) — no AUTOINCREMENT/SERIAL dialect.
 *  - TEXT for timestamps (ISO-8601) and JSON blobs.
 *  - INTEGER 0/1 for booleans.
 *  - REAL for numerics.
 *  - ON CONFLICT upserts (supported by both modern SQLite and Postgres).
 *
 * Every table carries id, created_at, source; updated_at and data_quality_score
 * where relevant, per spec.
 */
